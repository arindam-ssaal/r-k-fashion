...
function DailyCashReport() {
  ...
  //! AG Grid State
  const [rowData, setRowData] = useState<DayCloseSummaryRow[]>([]);
  ...
  const handleExcelPrint = async () => {
    ...
    try {
      ...
      let responseData: DayCloseSummaryRow[] = response?.data || [];
      ...
      let formattedData = responseData.map((item) => ({
        ...item,
        SessionCloseDate: item.SessionCloseDate ? format(new Date(item.SessionCloseDate), 'dd-MM-yyyy') : '',
        EntryDate: item.EntryDate ? format(new Date(item.EntryDate), 'dd-MM-yyyy') : '',
      }));
      ...
      setRowData([]);
      setPrint(false);
      toast.error('Failed to fetch day close summary report', {
        style: { backgroundColor: '#f7edeb', color: '#ff6242' },
      });
    } finally {
      setPrinting(false);
  ...
  const handleDownloadExcel = () => {
    ...
    saveAs(blob, `DailyCashReport_${fromDate}_to_${toDate}.xlsx`);
  };
