...
function StockSaleReportExcel() {
  ...
  //! AG Grid State
  const [rowData, setRowData] = useState<VoucherDataRow[]>([]);
  const [colDefs] = useState<ColDef[]>([
    ...
    {
      field: 'SessionOpenDate',
      headerName: 'Session Open Date',
      filter: false,
      floatingFilter: false,
      width: 180,
  ...
  const __handleExcelPrint = async () => {
    ...
    try {
      ...
      let response = await GetAPI(
        '/api/Report/VoucherReport',
        PType,
        PJsonData,
        cookies
      );

      let responseData: VoucherDataRow[] = response?.data || [];
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `VoucherReport_${fromDateFormatted}_to_${toDateFormatted}.xlsx`;
  ...
  const ____fetchMemoWiseData = async (page: number, size: number) => {
    const fromDateFormatted = formatApiDate(fromDate);
    const toDateFormatted = formatApiDate(toDate);
