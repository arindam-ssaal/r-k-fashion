...
export function useCreateUsermasterGeneral(){
    ...
    >({
      mutationFn: async (userMasterData: UserMasterPostType) => {
        setIsLoading(true) 
        return await userMasterClient.createUserMasterGeneral(userMasterData)
      },
      onSuccess: () => {
    ...
    return { createUsermasterGeneral: mutateAsync, data, error, isPending }
}