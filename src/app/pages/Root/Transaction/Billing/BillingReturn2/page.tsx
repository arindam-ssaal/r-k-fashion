...
const BillingReturn2 = () => {
  ...
  */
  ...
  const handleEnterPress = (event) => {
    if (event.key === 'Enter') {
      ...
      setBarCodeInput('')
      //! Blank Enter -> show all invoices again
  ...
  //! Scan / type an item -> invoice list shows only the invoices that contain it
  ...
  */
  ...
  //! Flat list of every item line across all listed invoices
  const buildAvailableItemList = async () => {
    ...
    if (failedCount > 0) {
      toast.warning(`Items of ${failedCount} invoice(s) could not be loaded. Scan again to retry.`, {
  ...
  //! Invoice list -> only invoices in billIDs (4 -> 3 / 2 ...)
  const applyItemInvoiceFilter = (billIDs: Set<string>, label: string) => {
    const currentSelected = (filteredSelectBillData as any[]).find((invoice: any) => invoice.selected)
    const selectedID = currentSelected ? String(currentSelected.billID) : ''
    const keepSelection = selectedID !== '' && billIDs.has(selectedID)

    const invoices = [...(selectBillListData as any[])]
      .reverse()
      .filter((invoice: any) => billIDs.has(String(invoice.billID)))
      .map((invoice: any) => ({
        ...invoice,
        selected: keepSelection && String(invoice.billID) === selectedID,
      }))

    // Selected invoice is not in the result -> clear it (same as un-ticking it)
    if (selectedID && !keepSelection) {
      setSelectedBillID(null)
      setSelectBillDetailsListData([])
      setFilteredSelectBillDetailsData([])
    }
    ...
    setItemFilterInfo({ label, count: invoices.length })
    setSelectedInvoiceRowIndex(selectedIndex >= 0 ? selectedIndex : 0)
    setIsFocusedOnItemTable(false)
    setTimeout(() => scrollToInvoiceRow(selectedIndex >= 0 ? selectedIndex : 0), 50)
  }

  //! Back to all invoices (4 / 5 ...)
  const clearItemSearchFilter = () => {
    ...
    const invoices = [...(selectBillListData as any[])]
      ...
      .map((invoice: any) => ({
        ...invoice,
        selected: selectedID !== '' && String(invoice.billID) === selectedID,
      }))
    const selectedIndex = invoices.findIndex((invoice: any) => invoice.selected)
  ...
  const handleSelectBarcodeItem = async (barCodeEntered: string) => {
    ...
    try {
      ...
      const matches = await findItemSearchMatches(searchText, list)

      if (matches.length === 0) {
        toast.error(
          `No invoice has item "${searchText}". Check the invoice date range.`,
          { style: ITEM_SEARCH_ERROR_STYLE }
        )
        return
      }

      const billIDs = new Set<string>(matches.map((row: any) => String(row.billID)))
      const itemNames = new Set(matches.map((row: any) => row.itemName || row.barcode))
      const label = itemNames.size === 1 ? String([...itemNames][0]) : `"${searchText}"`

      applyItemInvoiceFilter(billIDs, label)
      toast.success(`${label} found in ${billIDs.size} invoice${billIDs.size === 1 ? '' : 's'}`, {
        style: ITEM_SEARCH_SUCCESS_STYLE,
      })

      // Leave the box so Up / Down / Space work on the invoice list straight away (ALT + I to come back)
      inputBarCodeRef.current?.blur()
    } catch (error) {
    ...
    <div className="relative z-30 min-h-screen w-full overflow-hidden p-4" style={{ backgroundColor: '#d6eaf8' }}>
      ...
      */}
      <div className="w-full px-2" style={{ backgroundColor: '#d6eaf8' }}>
        ...
        {/* Scan Item Section - Billing @ Return (filters the invoice list) */}
        <div className="p-2 rounded shadow-sm border mb-3">
          <div className="flex flex-wrap items-center gap-2 text-sm">
            ...
            {itemFilterInfo && (
              <span className="text-xs bg-amber-100 border border-amber-300 text-amber-900 px-2 py-1 rounded-md">
                {itemFilterInfo.count} of {(selectBillListData as any[]).length} invoices have{' '}
            ...
            <Button
              ...
              disabled={!itemFilterInfo}
              title="Show all invoices again"
              className="bg-rose-900 hover:bg-rose-700 hover:shadow-lg text-white text-xs px-4 h-[38px] transition-all duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <RotateCcw className="w-3 h-3 mr-1" />
              All Invoices ({(selectBillListData as any[]).length})
            </Button>
          </div>
        </div>
