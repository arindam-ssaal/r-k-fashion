...
const BillingRequest2 = () => {
  ...
  const currentRecallBillData = filteredRecallBillData.slice(
  ...
  const recalculateCashAndRefund = (updated) => {
    ...
    // Find tender amount (from Cash payment mode)
    ...
    // If tender not entered → keep everything zero
    if (!tender || tender <= 0) {
      if (cashIndex >= 0) {
        updated[cashIndex].amount = 0 // cash paid field disabled but set from auto
        updated[cashIndex].refundAmount = 0
      }
      return updated
    }

    // Total non-cash = Card + PhonePe + Credit Note
    let nonCashTotal = 0
    ...
    // Cash auto calculation
    const autoCash = bill - nonCashTotal
    const finalCash = autoCash < 0 ? 0 : autoCash // should not go negative

    // Refund calculation
    const refund = tender - finalCash

    // Apply to Cash row
    if (cashIndex >= 0) {
      updated[cashIndex].amount = finalCash
      updated[cashIndex].refundAmount = refund > 0 ? refund : 0
    }
            ...
            <div className="w-full p-4 mt-8 bg-white shadow-xl rounded-2xl border border-gray-200">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-4 border p-3 rounded-lg shadow-sm">
                  ...
                  <input
                    ...
                    className="flex-1 p-2 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
                    disabled
                  />
                </div>
                {/* 
                  ...
                  paymentListData
                    ...
                    .map((payment, index) => {
                      ...
                      const showCNRSelectButton =
                        name.includes('credit note received') || name.includes('creditnotereceived')
                      const showTenderAmtButton = name.includes('cash')
                ...
                <div className="border p-3 rounded-lg shadow-sm mt-2 flex">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    ...
                    <div className="flex flex-col">
                      <label className="text-sm font-medium mb-1">Coupon</label>{' '}
              ...
              <div className="flex justify-between mt-8">
                ...
                <Button
                  ...
                  className={`px-5 py-2 rounded-lg shadow-md ${totalBillAmount > 0 ? 'bg-rose-900 text-white hover:bg-rose-800' : 'bg-gray-400 text-gray-200 cursor-not-allowed'}`}
...
//! Backup till 3-12-2025 morning before checkout design changes