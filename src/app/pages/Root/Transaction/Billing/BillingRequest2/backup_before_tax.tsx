...
const BillingRequest2 = () => {
      ...
      if (updatedScannedItemListDataWRTPromo.length > 0) {
        ...
        if (
          ...
          updatedItems = updatedItems.map((item) => {
            ...
            const taxRateUpdated =
              effectiveMrp <= item.gstUpperLimit ? item.taxRate : item.gstSlabRate
            const taxAmount = (
  ...
  const handleApplyPromotion = async () => {
    ...
    if (
      ...
      Object.keys(selectedPromotionData).length > 0 &&
      Array.isArray(scannedItemListData) &&
      ...
      if (
        storedPromotion &&
        Object.keys(storedPromotion).length > 0 &&
        selectedPromotionID === storedPromotion?.promotionID
      ) {
        ...
        if (!hasEmptyDiscount) {
          ...
          const updatedScannedItemListData = scannedItemListData.map((item) => {
            const { mrp, quantity, taxRate, gstUpperLimit, gstSlabRate } = item
            const taxRateUpdated = mrp <= gstUpperLimit ? taxRate : gstSlabRate
            const taxAmount = ((mrp / (100 + taxRateUpdated)) * taxRateUpdated * quantity).toFixed(
              2
            )
            const taxableAmount = ((mrp / (100 + taxRateUpdated)) * 100 * quantity).toFixed(2)
        ...
        const hasEmptyPromo = updatedScannedItemListDataWRTDiscnt.some(
        ...
        /*
          ...
          */
        //const updatedScannedItemListData = scannedItemListData.map((scannedItem) => {
        const updatedScannedItemListData = updatedScannedItemListDataWRTDiscnt.map(
          (scannedItem) => {
            const promoItem = response.data.find((promo) => promo.itemCode === scannedItem.itemCode)
            if (promoItem) {
    ...
    setScannedItemListData((prevList) => {
      ...
      matchedItems.forEach((matchedItem) => {
        ...
        if (existingIndex !== -1) {
          ...
          const taxAmount = ((mrp / (100 + taxRateUpdated)) * taxRateUpdated * newQuantity).toFixed(
            2
          ) //(((mrp * taxRateUpdated) / 100) * newQuantity).toFixed(2);
  ...
  const recalculateCashAndRefund = (updated) => {
    ...
    const cashIndex = updated.findIndex((p) => p.availablePaymentmethod?.toLowerCase() === 'cash')
    const tender = cashIndex >= 0 ? parseFloat(updated[cashIndex].tenderAmount) || 0 : 0

    if (cashIndex < 0) return updated
    ...
    updated[cashIndex].amount = finalCash
    updated[cashIndex].refundAmount = refund
  ...
  const handleSaveBill = async () => {
    ...
    try {
      ...
      if (response.data[0].returnCode === 'Y') {
        ...
        setCheckoutSaveBillType('Bill')
        //setIsCheckoutSaveModalOpen(true);
        handleBillPrint(response.data[0].returnDocEntry)
      } else if (response.data[0].returnCode === 'F') {
        toast.error(response.data[0].returnMsg, {
          style: { backgroundColor: '#f7edeb', color: '#ff6242' },
        })
    ...
    <div className="relative z-30 bg-gray-50 min-h-screen w-full overflow-hidden">
        ...
        <div className="flex gap-2 w-full">
          ...
          <div className="flex-1 space-y-2">
                  ...
                  <TableBody>
                    {scannedItemListData.length === 0 ? (
                      ...
                      scannedItemListData.map((item, index) => (
                        <TableRow
                          key={index + 1}
                          className={`w-20 h-8 cursor-pointer transition-all duration-200 ${
            ...
            <div className="bg-white p-2 rounded shadow-sm border">
              ...
              <div>
                <div className="flex items-center gap-3 mt-4 mb-4 bg-gradient-to-r from-gray-50 to-white p-4 rounded-xl shadow-sm border border-gray-200">
                  ...
                  <div className="flex-1">
                    ...
                    <textarea
                      className="w-full h-16 px-3 py-2 border border-gray-300 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm placeholder-gray-400 bg-white shadow-sm"
      ...
      {showCheckOutModal && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          ...
          <div className="bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-50 shadow-2xl w-full h-full flex flex-col">
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-4">
              <div className="bg-white shadow-sm border border-sky-100 p-4">
                <div className="mb-3 pb-3 border-b border-sky-200">
                  <div className="flex items-center gap-3 bg-gradient-to-r from-sky-50 to-blue-50 p-3 border border-sky-200">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {loading ? (
                    ...
                    paymentListData
                      .filter((payment) => payment.availablePaymentmethod !== 'creditNoteIssued')
                      .map((payment, index) => {
                        /*
                        ...
                        return (
                          <div
                            key={index}
                            className="bg-white border border-sky-100 p-2 shadow-sm hover:shadow-md transition-shadow"
                          >
                            <div className="flex flex-col gap-1">
                              <div className="flex items-center gap-1">
                                <div className="p-1 bg-sky-100">
                                  <PaymentIcon className="w-3 h-3 text-sky-600" />
                                </div>
                  ...
                  {/* Coupon Section */}
                  <div className="bg-white border border-sky-100 p-3 shadow-sm mt-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="flex items-end">
                        <Button
                          className={`px-3 py-2 text-xs shadow-md ${
                            isCouponApplied
            ...
            {/* Footer */}
            <div className="bg-white border-t border-sky-200 p-3">
              <div className="flex justify-center items-center gap-3">
                ...
                <div className="flex-1 max-w-xs">
                  <div className="bg-gradient-to-r from-sky-50 to-blue-50 p-2 border border-sky-200">
                    <div className="space-y-1">
                ...
                <Button
                  ...
                  className={`px-4 py-2 text-sm shadow-md text-white ${
                    totalBillAmount > 0
