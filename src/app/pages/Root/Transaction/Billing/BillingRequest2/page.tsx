...
//! [RN] ============ Replacement Note config - change ONLY here later ============
const RN_CONFIG = {
  // Unadjusted Replacement Note list API
  // TEMP: using Credit Note API. Later change to '/api/PayMode/GetAllUnAdjustReplacementNote'
  LIST_API: '/api/PayMode/GetAllUnAdjustCreditNote',
  // PostSaleBill field for Bill Type (backend will add it later; change key if name differs)
  BILL_TYPE_KEY: 'billType',
  // Bill Type value for Replacement Note transaction (client point 6)
  BILL_TYPE_VALUE: 'E',
}
//! [RN] =========================================================================
...
const BillingRequest2 = () => {
  ...
  // [RN] Replacement Note Adjustment states
  const [rnSearchText, setRNSearchText] = useState('')
  const [rnListData, setRNListData] = useState([])
  const [filteredRNData, setFilteredRNData] = useState([])
  const [rnCurrentPage, setRNCurrentPage] = useState(1)
  const rnPerPage = 5
  const [selectedRNList, setSelectedRNList] = useState([])
  const [storedRN, setStoredRN] = useState(null)
  const [hasUnAdjustReplacementNote, setHasUnAdjustReplacementNote] = useState(false)
  ...
  // [RN] Paymode type helper - checks both availablePaymentmethod and paymentModeName
  // Returns: 'cash' | 'card' | 'upi' | 'cnAdjust' | 'cnIssued' | 'rnAdjust' | 'rnIssued' | 'other'
  const normalizePayModeText = (value) =>
    (value || '').toString().toLowerCase().replace(/[\s_-]+/g, '')
  const getPayModeType = (payment) => {
    const method = normalizePayModeText(payment?.availablePaymentmethod)
    const modeName = normalizePayModeText(payment?.paymentModeName)
    const has = (...keys) => keys.some((k) => method.includes(k) || modeName.includes(k))

    if (has('replacement')) return has('issue') ? 'rnIssued' : 'rnAdjust'
    if (has('creditnote')) return has('issue') ? 'cnIssued' : 'cnAdjust'
    if (has('cash')) return 'cash'
    if (has('creditcard', 'debitcard', 'debit', 'card')) return 'card'
    if (has('phonepe', 'phonepay', 'upi', 'mobilewallet', 'paytm', 'gpay', 'wallet')) return 'upi'
    return 'other'
  }
  const isReplacementNoteAdjustPayment = (payment) => getPayModeType(payment) === 'rnAdjust'
  const isReplacementNoteIssuedPayment = (payment) => getPayModeType(payment) === 'rnIssued'
  ...
  const handleCNReceivedSelection = (cnID, cnData) => {
    // [RN] TEMP guard: same document cannot be used in both CN and RN
    const usedInRN = [...(selectedRNList || []), ...(storedRN || [])].some(
      (rn) => rn.billID === cnID
    )
    if (usedInRN && !selectedCNReceivedList.some((item) => item.billID === cnID)) {
      toast.error('This note is already selected in Replacement Note Adjustment.', {
        style: { backgroundColor: '#f7edeb', color: '#ff6242' },
      })
      return
    }
  ...
  // [RN] ---------------- Replacement Note Adjustment ----------------
  const REPLACEMENT_NOTE_LIST_API = RN_CONFIG.LIST_API // change in RN_CONFIG (top of file)
  const errorToastStyle = { style: { backgroundColor: '#f7edeb', color: '#ff6242' } }

  const setReplacementNoteAmount = (amount) => {
    setPaymentListData((prev) =>
      recalculateCashAndRefund(
        prev.map((payment) =>
          isReplacementNoteAdjustPayment(payment) ? { ...payment, amount } : payment
        )
      )
    )
  }

  const resetReplacementNoteState = () => {
    setRNSearchText('')
    setRNListData([])
    setFilteredRNData([])
    setRNCurrentPage(1)
  ...
  const fetchReplacementNoteAvailabilityForCustomer = async (customerID) => {
    if (!customerID) {
      setHasUnAdjustReplacementNote(false)
      return
    }
    try {
      const responseJson = await GetAPI(
        REPLACEMENT_NOTE_LIST_API,
        '?CustomerID=' + customerID,
        {},
        cookies
      )
      const hasRN = (responseJson?.data || []).length > 0
      setHasUnAdjustReplacementNote(hasRN)
      if (!hasRN) {
        resetReplacementNoteState()
        setReplacementNoteAmount('')
      }
    } catch (error) {
      setHasUnAdjustReplacementNote(false)
      resetReplacementNoteState()
  ...
  const fetchRNListData = async () => {
    try {
      ...
      setRNListData(responseJson?.data || [])
      setFilteredRNData(responseJson?.data || [])
    } catch (error) {
      setRNListData([])
      setFilteredRNData([])
      toast.error(
        `Replacement Note list could not be loaded (${REPLACEMENT_NOTE_LIST_API}). ${error?.message || ''}`,
        errorToastStyle
      )
    } finally {
      setLoading(false)
  ...
  const handleRNSearch = (searchText) => {
    setRNSearchText(searchText)
    setRNCurrentPage(1)
    const text = searchText.trim().toLowerCase()
    if (!text) {
      setFilteredRNData(rnListData)
      return
    }
    setFilteredRNData(
      rnListData.filter(
        (rn) =>
          (rn.billNo || '').toLowerCase().includes(text) ||
          (rn.billDate || '').includes(searchText.trim()) ||
          String(rn.amount ?? '').includes(searchText.trim())
      )
    )
  }
  const totalRNPages = Math.max(1, Math.ceil(filteredRNData.length / rnPerPage))
  const currentRNData = filteredRNData.slice(
    (rnCurrentPage - 1) * rnPerPage,
    rnCurrentPage * rnPerPage
  )
  const totalSelectedRNAmount = selectedRNList.reduce(
    (sum, rn) => sum + (Number(rn.amount) || 0),
    0
  )
  const totalAppliedRNAmount = (storedRN || []).reduce(
    (sum, rn) => sum + (Number(rn.amount) || 0),
    0
  )
  ...
  const handleRNClick = () => {
    if (!storedCustomer?.customerID) {
      toast.error('Please select a customer first.', errorToastStyle)
      return
    }
    setSelectedRNList(storedRN || [])
    setShowRNModal(true)
    fetchRNListData()
  }

  const handleRNSelection = (rnData) => {
    // [RN] TEMP guard: same document cannot be used in both CN and RN
    const usedInCN = [...(selectedCNReceivedList || []), ...(storedCNReceived || [])].some(
      (cn) => cn.billID === rnData.billID
    )
    if (usedInCN && !selectedRNList.some((rn) => rn.billID === rnData.billID)) {
      toast.error('This note is already selected in Credit Note Adjustment.', errorToastStyle)
      return
    }
    setSelectedRNList((prev) =>
      prev.some((rn) => rn.billID === rnData.billID)
        ? prev.filter((rn) => rn.billID !== rnData.billID)
        : [...prev, rnData]
    )
  }

  const handleApplyRN = () => {
    const billAmt = Number(totalBillAmount) || 0
    // Client point 5: Bill Net Payable must be >= Replacement Note amount
    if (billAmt < totalSelectedRNAmount) {
      toast.error(
        `Bill amount (Rs. ${billAmt}) must be equal to or greater than Replacement Note amount (Rs. ${totalSelectedRNAmount}).`,
        errorToastStyle
      )
      return
    }
    setStoredRN(selectedRNList)
    setReplacementNoteAmount(totalSelectedRNAmount) // full RN value goes to the textbox
    setShowRNModal(false)
  }

  const handleClearAndCloseRN = () => {
    setShowRNModal(false)
    resetReplacementNoteState()
    setReplacementNoteAmount('')
  }
  // [RN] ---------------- End Replacement Note Adjustment ----------------
  ...
  useEffect(() => {
    ...
    setPaymentListData((prev) =>
      prev.map((payment) =>
        isCreditNoteReceivedPayment(payment.availablePaymentmethod) ||
        isReplacementNoteAdjustPayment(payment) // [RN]
          ? { ...payment, amount: '' }
          : payment
      )
    )
    resetReplacementNoteState() // [RN]

    fetchCreditNoteAvailabilityForCustomer(storedCustomer?.customerID)
    fetchReplacementNoteAvailabilityForCustomer(storedCustomer?.customerID) // [RN]
    ...
    setPaymentListData((prev) =>
      prev.map((payment) => {
        ...
        const shouldResetAmount =
          ...
          ['card', 'upi', 'cnAdjust', 'rnAdjust'].includes(getPayModeType(payment)) // [RN]
  ...
  const recalculateCashAndRefund = (updated) => {
    ...
    normalizedUpdated.forEach((p) => {
      // [RN] type based - works for Card / PhonePe / Credit Note / Replacement Note
      const payType = getPayModeType(p)
      if (['card', 'upi', 'cnAdjust', 'rnAdjust'].includes(payType)) {
  ...
  const handleSaveBill = async () => {
    ...
    // [RN] Client point 5: Bill Net Payable must be >= Replacement Note amount
    if (totalAppliedRNAmount > 0 && (Number(totalBillAmount) || 0) < totalAppliedRNAmount) {
      toast.error(
        'Bill amount must be equal to or greater than Replacement Note amount!',
        errorToastStyle
      )
      return
    }
    ...
    const objPayment = paymentListData
      .filter(
        (payment) =>
          ...
          payment.availablePaymentmethod.toLowerCase() !== 'creditnotereceived' &&
          payment.availablePaymentmethod.toLowerCase() !== 'creditnoteissued' &&
          !isReplacementNoteAdjustPayment(payment) && // [RN] sent separately below
          !isReplacementNoteIssuedPayment(payment) // [RN]
      )
      .map((payment, index) => ({
        billID: 0,
        lineNum: index + 1,
    ...
    // [RN] Replacement Note Adjustment payment lines (one line per RN, full value)
    const rnPayMode = paymentListData.find((payment) => isReplacementNoteAdjustPayment(payment))
    const objPayment3 = (storedRN || [])
      .filter((rn) => (Number(rn.amount) || 0) > 0)
      .map((rn, index) => ({
        billID: 0,
        lineNum: objPayment.length + objPayment2.length + index + 1,
        modeOfPayementID: rnPayMode?.paymentModeID || 0,
        modeOfPaymentName: rnPayMode?.paymentModeName || '',
        value: Number(rn.amount) || 0,
        upItransactionID: '',
        cardNo: '',
        cardHolderName: '',
        cardValidity: '',
        ...
        forexAmt: 0,
        forexBalance: 0,
        returnDocumentID: rn?.billID || 0,
        returnDocumentNo: rn?.billNo || '',
        tenderAmount: 0,
      }))
    ...
    const formData = {
      ...
      objPayment: [...objPayment, ...objPayment2, ...objPayment3], // [RN] objPayment3 added
      //! [RN] Client point 6: Bill Type 'E' for Replacement Note transaction
      //! key/value from RN_CONFIG (top of file)
      ...(objPayment3.length > 0
        ? { [RN_CONFIG.BILL_TYPE_KEY]: RN_CONFIG.BILL_TYPE_VALUE }
        : {}),
    }
    ...
    const handleKeyDown = (event) => {
      ...
      if (showCheckOutModal && event.altKey) {
        ...
        paymentListData.forEach((payment, index) => {
          // [RN] type based - Card => Alt + B, PhonePe/UPI => Alt + M
          const payType = getPayModeType(payment)

          if (key === 'b' && payType === 'card') {
            event.preventDefault()
            toggleNoteField(index)
          }

          if (key === 'm' && payType === 'upi') {
            event.preventDefault()
            toggleNoteField(index)
          }
        })
  ...
  return (
    <div className="light-theme relative z-30 min-h-screen w-full overflow-hidden" style={{ backgroundColor: '#d6eaf8' }}>
              ...
              <div className="bg-white p-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {loading ? (
                    ...
                    paymentListData
                      .map((payment, index) => ({ payment, index })) // [RN] keep original index
                      .filter(({ payment }) => {
                        const payType = getPayModeType(payment)
                        if (payment.availablePaymentmethod === 'creditNoteIssued') return false
                        if (payType === 'cnIssued' || payType === 'rnIssued') return false // [RN] hide Issued
                        if (payType === 'rnAdjust') return true // [RN] always show RN Adjustment
                        return (
                      ...
                      .map(({ payment, index }) => {
                        ...
                        // [RN] type based detection (Card / PhonePe / Replacement Note)
                        const payType = getPayModeType(payment)
                        const isCard = payType === 'card'
                        const isUPI = payType === 'upi'
                        const isRNAdjust = payType === 'rnAdjust'
                        const showNoteButton = isCard || isUPI
                        const showCNRSelectButton =
                          hasUnAdjustCreditNote &&
                          (name.includes('credit note received') ||
                            name.includes('creditnotereceived'))
                        const showRNSelectButton = isRNAdjust // [RN] always show Select
                        const isCashTenderAmt = name.includes('cash')
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <input
                                ...
                                disabled={
                                  ...
                                  payment.availablePaymentmethod?.toLowerCase() ===
                                  'creditnotereceived' ||
                                  isRNAdjust // [RN] value only via Select popup
                                }
                              />
                              <div className="flex gap-1">
                                ...
                                {/* [RN] Replacement Note Adjustment - Select button */}
                                {showRNSelectButton && (
                                  <Button
                                    className="bg-sky-500 text-white px-1.5 py-0.5 text-[10px] hover:bg-sky-600 whitespace-nowrap shadow-sm flex-1"
      ...
      {/* [RN] 'Replacement Note Adjustment' Modal */}
      {showRNModal && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-8/12">
            <h2 className="text-xl font-semibold mb-4">Replacement Note</h2>
            <input
              type="text"
              placeholder="Search Replacement Note..."
              value={rnSearchText}
              onChange={(e) => handleRNSearch(e.target.value)}
              className="w-60 p-2 border border-gray-300 rounded-md mb-4"
            />
            <div className="overflow-y-auto max-h-96">
              <div className="overflow-x-auto border rounded-md w-full">
                <table className="w-full border-collapse border border-gray-300 text-sm">
                  <thead>
                    <tr className="bg-gray-200 h-10 text-left">
                      <th className="border px-2">Select</th>
                      <th className="border px-2">RN No.</th>
                      <th className="border px-2">RN Date</th>
                      <th className="border px-2">Customer Name</th>
                      <th className="border px-2 text-right">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    ) : currentRNData?.length ? (
                      currentRNData.map((item) => (
                        <tr key={item.billID} className="h-10">
                          <td className="border px-2 text-center">
                            <input
                              type="checkbox"
                              checked={selectedRNList.some((rn) => rn.billID === item.billID)}
                              onChange={() => handleRNSelection(item)}
                            />
                          </td>
                          <td className="border px-2">{item.billNo}</td>
                          <td className="border px-2">{(item.billDate || '').split(' ')[0]}</td>
            ...
            <div className="flex justify-between items-center mt-4">
              <Button
                onClick={() => setRNCurrentPage((prev) => Math.max(prev - 1, 1))}
                disabled={rnCurrentPage === 1}
              >
                Previous
              </Button>
              <span>
                Page {rnCurrentPage} of {totalRNPages}
              </span>
              <Button
                onClick={() => setRNCurrentPage((prev) => Math.min(prev + 1, totalRNPages))}
                disabled={rnCurrentPage === totalRNPages}
              >
                Next
              </Button>
            </div>
            <div className="mt-4 flex justify-between items-center gap-4">
              <span className="text-sm">
                Selected RN Amount: <b>{totalSelectedRNAmount}</b> | Bill Amount:{' '}
                <b>{totalBillAmount}</b>
              </span>
              <div className="flex gap-4">
                <Button
                  className="px-4 py-1 bg-green-600 text-white rounded-md hover:bg-green-700"
                  onClick={handleApplyRN}
                  disabled={selectedRNList.length === 0}
                >
                  Apply
                </Button>
                <Button
                  className="px-4 py-1 bg-gray-600 text-white rounded-md hover:bg-gray-700"
                  onClick={handleClearAndCloseRN}
                >
                  Clear & Close
                </Button>
              </div>
